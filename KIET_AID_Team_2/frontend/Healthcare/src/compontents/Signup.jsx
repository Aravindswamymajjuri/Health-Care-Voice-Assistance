    ...
    <div className={`auth-container signup ${isAnimating ? 'active' : ''}`}>
      ...
      <div className="auth-card">
        <div className="auth-hero">
          ...
          <h2 className="hero-title">{t('register')}</h2>
          <p className="hero-sub">{t('joinUs')}</p>
        </div>


        {/* Stepper */}
        <div className="signup-stepper" role="tablist" aria-label="Signup steps">
          <div role="tab" tabIndex={0} onClick={() => handleStepClick(0)} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') handleStepClick(0); }} className={`step ${step === 0 ? 'active' : step > 0 ? 'completed' : ''}`}>{t('account')}</div>
          <div role="tab" tabIndex={0} onClick={() => handleStepClick(1)} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') handleStepClick(1); }} className={`step ${step === 1 ? 'active' : step > 1 ? 'completed' : ''}`}>{t('personal')}</div>
          <div role="tab" tabIndex={0} onClick={() => handleStepClick(2)} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') handleStepClick(2); }} className={`step ${step === 2 ? 'active' : step > 2 ? 'completed' : ''}`}>{t('medical')}</div>
          <div role="tab" tabIndex={0} onClick={() => handleStepClick(3)} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') handleStepClick(3); }} className={`step ${step === 3 ? 'active' : ''}`}>{t('review')}</div>
        </div>
