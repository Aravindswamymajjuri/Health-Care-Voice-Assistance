...
const HealthcareChatbot = ({ currentUser, onLogout }) => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      // Add timeout: if WebSocket doesn't connect within 5 seconds, close it and use REST fallback
      wsTimeout = setTimeout(() => {
        if (ws && ws.readyState === WebSocket.CONNECTING) {
          console.warn('WebSocket connection timeout (5s) — falling back to REST API');
          ws.close();
          setTipsSource('local');
        }
      }, 5000);
      ...
      ws.onclose = () => {
        clearTimeout(wsTimeout);
        console.warn('⏹️ WebSocket closed — using local tips fallback');
        setTipsSource('local');
      };

      ws.onerror = (e) => {
        clearTimeout(wsTimeout);
        console.warn('⚠️ WebSocket error — falling back to REST API for tips', e);
        setTipsSource('local');
      };

      wsRef.current = ws;
    } catch (e) {
      console.warn('Failed to open WebSocket for tips — using REST fallback', e);
      setTipsSource('local');
    }
  ...
  const sendTextMessage = async (text) => {
    ...
    try {
      ...
      const wellnessReminder = generateWellnessReminder(context);
      const encouragement = getEncouragementMessage(context);
      ...
      console.log('📤 Sending text to backend with context:', { context });
      ...
      if (response.data.status === 'success') {
        ...
        const enhancedResponse = enhanceResponseWithContext(responseText, context);
            ...
            {showEmergencyEmailConfirm && (
              <div className="emergency-email-confirm" role="dialog" aria-modal="true">
                <div className="confirm-panel">
                  <h4>Emergency Alert Confirmation</h4>
                  <p><strong>Alert Type:</strong> {emergencyType.toUpperCase()}</p>
                  <p style={{ fontSize: '12px', color: '#666', marginTop: '8px' }}>
                    📨 The entire chat history ({messages.length} messages) will be sent to your emergency contact.
                  </p>
                  ...
                  <label style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 12, fontSize: '13px' }}>
                    <input type="checkbox" checked={autoSendNext} onChange={(e) => setAutoSendNext(e.target.checked)} />
                    <span>✓ Remember this email and auto-send on future emergencies</span>
                  </label>

                  <div style={{ display: 'flex', gap: 12, justifyContent: 'flex-end', marginTop: 20 }}>
                    <button className="toolbar-btn" onClick={() => setShowEmergencyEmailConfirm(false)} style={{ background: '#f0f0f0', color: '#333', padding: '10px 24px' }}>
                      Cancel
                    </button>
                    <button
                      className="toolbar-btn"
                      onClick={async () => {
                        try {
                          setSendingEmergencyEmail(true);
                          const convId = activeConversationId || (conversations[0] && conversations[0].conversation_id) || null;
                          // Send ENTIRE chat history, not just last 50
                          const allMessages = messages.map(m => ({ type: m.type, text: m.text, timestamp: m.timestamp }));
                          ...
                          toast.success(resp.data?.message || '🚨 Emergency alert sent to ' + emergencyEmailInput);
                          setShowEmergencyEmailConfirm(false);
                        } catch (err) {
                      ...
                      style={{ background: '#ff1744', color: 'white', padding: '10px 24px', fontWeight: 'bold' }}
                      disabled={sendingEmergencyEmail}
                    >
                      {sendingEmergencyEmail ? '⏳ Sending...' : '✉️ Send Emergency Alert'}
                    </button>
                  </div>
                </div>
              </div>
          ...
          <div className="dashboard-panel">
            ...
            <div className="dashboard-grid">
              <div className="stat-card">
                ...
                <div className="stat-label">{t('totalConversations')}</div>
              </div>
              <div className="stat-card">
              ...
              <div className="stat-card">
                ...
                <div className="stat-label">{t('voiceQueries')}</div>
              </div>
              <div className="stat-card">
        ...
        <div className="messages-container" ref={messagesContainerRef}>
          {messages.map((msg) => (
            <div key={msg.id}>
              ...
              {/* PersonalizedInsights removed — deprecated component */}
            </div>
          ))}
          {isLoading && (
